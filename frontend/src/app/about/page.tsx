    ...
    <div className="py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto mb-12 text-justify space-y-5">
          <p className="text-[17px] text-[#4A4845] leading-relaxed">
        ...
        <div className="space-y-5 max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 gap-5">
            <div className="card-base p-6">
              ...
              <p className="text-[16px] text-[#6B6860] leading-relaxed">{vision?.content || "To become a centre for excellence in stroke rehabilitation"}</p>
            </div>
            <div className="card-base p-6">
              <div style={{ height: "3px", background: "#B84A18", borderRadius: "3px 3px 0 0", margin: "-24px -24px 16px -24px" }} />
              <h2 className="font-display text-2xl text-[#1C1C1A] mb-3">{mission?.title || "Mission"}</h2>
              <p className="text-[16px] text-[#6B6860] leading-relaxed">{mission?.content || "To conduct high quality research, training programs and to deliver comprehensive stroke rehabilitation services"}</p>
            </div>
          </div>
          <div className="card-base p-6">
            <div style={{ height: "3px", background: "#B84A18", borderRadius: "3px 3px 0 0", margin: "-24px -24px 16px -24px" }} />
            <h2 className="font-display text-2xl text-[#1C1C1A] mb-3">{objectives?.title || "Objectives"}</h2>
            <ul className="text-[16px] text-[#6B6860] space-y-2">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-8 items-center">
            {nationalCollaborations.map((collab) => (
              collab.logo === "/placeholder.svg" ? (
                <span key={collab.name} className="text-[14px] text-[#6B6860] font-medium text-center leading-snug">{collab.name}</span>
              ) : (
                <Image
                  key={collab.name}
                  src={collab.logo}
