...
export default function ProjectCard({ project, collaborative }: ProjectCardProps) {
    ...
    return (
      <div className="card-base bg-white p-5">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className={`text-[12px] font-semibold uppercase tracking-wide text-[#6B6860] px-2 py-0.5 ${badgeBg} rounded-full ml-auto`}>
            {collaborative ? "Collaborative" : "Funded"}
          </span>
        </div>
        <h3 className="font-semibold text-[#1C1C1A] text-[16px] leading-snug">{project.title}</h3>
        <div className="mt-3 space-y-1.5 text-[14px] text-[#6B6860]">
  ...
  if (isPhD(project)) {
    return (
      <div className="card-base bg-white p-5">
        ...
        <div className="flex items-center gap-2 mb-3">
          <StatusBadge status={project.status} />
          <span className="text-[12px] font-semibold tracking-wide text-[#6B6860] px-2 py-0.5 bg-[#F5F0FF] rounded-full ml-auto">
            PhD
          </span>
        </div>
        <h3 className="font-semibold text-[#1C1C1A] text-[16px] leading-snug">{project.thesisTitle}</h3>
        <div className="mt-3 space-y-1.5 text-[14px] text-[#6B6860]">
  ...
  return (
    <div className="card-base bg-white p-5">
      ...
      <div className="flex items-center gap-2 mb-3">
        <StatusBadge status={project.status} />
        <span className="text-[12px] font-semibold uppercase tracking-wide text-[#6B6860] px-2 py-0.5 bg-[#F0FDF4] rounded-full ml-auto">
          {project.category}
        </span>
      </div>
      <h3 className="font-semibold text-[#1C1C1A] text-[16px] leading-snug">{project.title}</h3>
